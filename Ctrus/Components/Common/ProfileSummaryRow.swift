...
struct ProfileSummaryContent: View {
  ...
  private var primaryTextColor: Color {
    forcedLight ? .fixedLightPrimaryText : .primary
  }
    ...
    HStack(spacing: layout == .dashboard ? 12 : 10) {
      VStack(alignment: .leading, spacing: contentSpacing) {
        ...
        if metadata.isVisible {
          ProfileSummaryMetadataLine(profile: profile, metadata: metadata, forcedLight: forcedLight)
        }

        if showsStatusLine {
          ProfileSummaryStatusLine(profile: profile, mode: statusMode, forcedLight: forcedLight)
        }
      }
  ...
  private var activeChip: some View {
    Text("Active")
      ...
      .padding(.vertical, 4)
      .background(
        forcedLight ? Color.black.opacity(0.08) : Color(.secondarySystemFill), in: Capsule())
  }
      ...
      HStack(spacing: 4) {
        ForEach(Array(dayLabels.enumerated()), id: \.offset) { _, label in
          Text(label)
            ...
            .foregroundStyle(
              forcedLight ? Color.fixedLightSecondaryText.opacity(0.7) : Color.secondary.opacity(0.7)
            )
            .frame(maxWidth: .infinity)
        }
      }
...
private struct ProfileSummaryStatusLine: View {
  ...
  var body: some View {
    if let schedule = profile.schedule, schedule.isActive {
      ProfileSummaryNextScheduleLine(schedule: schedule, forcedLight: forcedLight)
    ...
    if !indicators.isEmpty {
      HStack(spacing: 12) {
        ForEach(Array(indicators.prefix(3)), id: \.self) { label in
          HStack(spacing: 5) {
            Circle()
              .fill((forcedLight ? Color.fixedLightPrimaryText : Color.primary).opacity(0.85))
              .frame(width: 5, height: 5)

            Text(label)
              .font(.caption2)
              .foregroundColor(forcedLight ? Color.fixedLightSecondaryText : Color.secondary)
              .lineLimit(1)
          }
        }
      }
