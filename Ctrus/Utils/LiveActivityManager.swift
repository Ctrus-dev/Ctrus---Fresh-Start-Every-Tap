...
class LiveActivityManager: ObservableObject {
  ...
  @Published var currentActivity: Activity<CtrusWidgetAttributes>?

  // Use AppStorage for persisting the activity ID across app launches
  @AppStorage("com.Ctrus.currentActivityId") private var storedActivityId: String = ""
  ...
  private func restoreExistingActivity() {
    ...
    if !storedActivityId.isEmpty {
      if let existingActivity = Activity<CtrusWidgetAttributes>.activities.first(where: {
  ...
  func endSessionActivity() {
    ...
    // End the activity
    let completedState = CtrusWidgetAttributes.ContentState(
      startTime: Date.now
    )
