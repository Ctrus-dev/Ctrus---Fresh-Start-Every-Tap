...
class ThemeManager: ObservableObject {
  ...
  @AppStorage(
    "CtrusThemeColorName", store: UserDefaults(suiteName: "group.mo.dev.Ctrus"))
  ...
  var pastelBackground: Color {
    ...
    return Color(UIColor(hue: hue, saturation: saturation * 0.22, brightness: 0.99, alpha: 1))
  }
}

extension Color {
  // Fixed light-mode text colors for screens (Home, Intro) that always render
  // against the pastel background regardless of the system's light/dark setting.
  static let fixedLightPrimaryText = Color(
    UIColor.label.resolvedColor(with: UITraitCollection(userInterfaceStyle: .light)))
  static let fixedLightSecondaryText = Color(
