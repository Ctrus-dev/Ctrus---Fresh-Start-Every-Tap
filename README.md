<p align="center">
  <img src="./images/Ctrus-logo.png" width="250" style="border-radius: 40px;" alt="Ctrus app icon">
</p>

<h1 align="center"><a href="https://apps.apple.com/ca/app/foqos/id6736793117">Ctrus</a></h1>

<p align="center">
  <strong>Free, open-source iPhone app blocker with NFC unlocking.</strong>
</p>

<p align="center">
  Ctrus is a privacy-first iOS app blocker that uses Apple's Screen Time API to block distracting apps and websites. Start a focus session manually, with an NFC tag, with a timer, or with temporary access rules. It is built as a free and open-source alternative to Brick, Unpluq, Blok, Opal, and other paid app blockers for iPhone.
</p>
...
Ctrus helps you put friction between yourself and the apps or websites you want to avoid. You create blocking profiles for work, study, bedtime, or any other routine, choose the apps and domains to block, then decide how that profile starts and stops.

Unlike hardware-first blockers, Ctrus works with cheap NFC tags, iOS Shortcuts, and in-app controls. Your data stays on your device. There is no account requirement, no tracking, no ads, and no subscription.

## Free Alternative to Brick, Unpluq, Blok, and Opal

Ctrus is built for people who want physical app blocking on iPhone without buying a dedicated device or paying for a subscription. It covers the core workflows people often look for in a Brick alternative, Unpluq alternative, Blok alternative, or Opal alternative: app blocking, website blocking, NFC unlocks, timed focus sessions, and local privacy.

Ctrus is not affiliated with Brick, Unpluq, Blok, or Opal.
...
Ctrus includes several ways to start and stop a blocking session. All strategies are implemented in `Ctrus/Models/Strategies/` and registered in `Ctrus/Utils/StrategyManager.swift`.

| Strategy | Best for | How it works |
| --- | --- | --- |
| `ManualBlockingStrategy` | Simple in-app blocking | Start and stop directly in Ctrus. |
...
Strict unlocks can require a specific approved NFC tag. When a profile has one configured, other tags cannot stop that profile.

## How It Works

1. Download Ctrus from the [App Store](https://apps.apple.com/ca/app/foqos/id6736793117).
2. Grant Screen Time access so Ctrus can block selected apps and websites.
3. Create a profile for the apps, categories, and domains you want to block.
4. Pick a blocking strategy: manual, NFC, timer, pause timer, or temporary access.
5. Start the session and let Ctrus enforce the block until your chosen stop condition is met.
...
Ctrus works with common NFC tags such as NTAG213. You can write profile links to NFC tags from inside the app, then place tags where they match the routine: desk, bedside table, gym bag, office door, or another physical location.

## iOS Shortcuts Setup

You can trigger Ctrus profiles through iOS Shortcuts. For NFC automations, create one automation per NFC tag.

1. Open the Shortcuts app and go to the Automation tab.
2. Tap Create Personal Automation and choose NFC.
3. Scan the NFC tag and name it for the Ctrus profile you want to run.
...
5. Add a blank automation, search for Ctrus, and add Check if Ctrus Session is Active. Turn off Show When Run.

<img width="250" alt="iOS Shortcuts action setup" src="/images/shortcut-instructions-2.png" />

6. Add an If block with Start Ctrus Profile and Stop Ctrus Profile. For both actions, select the target Ctrus profile.
...
Ctrus also has a printable NFC brick and keychain design for 25 mm NFC tags:

- [Ctrus NFC Brick and Keychain on Printables](https://www.printables.com/model/1537982-foqos-nfc-brick-keychain)

<img width="500" alt="3D printable Ctrus NFC brick and keychain" src="/images/Ctrus-brick-keychain.png" />

## Android Alternative

Ctrus is iOS-only. Android users looking for a similar physical app blocker can try [Switchly](https://switchly.saltyy.at/#features) or [Lock](https://lock-app.fr) ([source](https://github.com/NathanLenias/lock-app)), a free and open-source NFC app blocker for Android.
...
```text
Ctrus/
|-- Ctrus/                     # Main app target
|   |-- Views/                 # SwiftUI views
|   |-- Models/                # SwiftData models and blocking strategies
|   |-- Components/            # Reusable UI components
|   |-- Utils/                 # Utility functions and managers
|   `-- Intents/               # App Intents and Shortcuts
|-- CtrusWidget/               # Widget and Live Activity extension
|-- CtrusDeviceMonitor/        # Device Activity monitor extension
|-- CtrusShieldAction/         # Managed Settings shield action extension
`-- CtrusShieldConfig/         # Managed Settings shield configuration extension
```
...
Ctrus is released under the MIT License. See [LICENSE](LICENSE) for details.
