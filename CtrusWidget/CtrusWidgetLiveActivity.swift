...
struct CtrusWidgetAttributes: ActivityAttributes {
...
struct CtrusWidgetLiveActivity: Widget {
  ...
  var body: some WidgetConfiguration {
    ActivityConfiguration(for: CtrusWidgetAttributes.self) { context in
      ...
      DynamicIsland {
        ...
        CtrusLogo(size: compactLogoSize)
  ...
  private func expandedIslandTitle(name: String) -> some View {
    HStack(alignment: .center, spacing: 8) {
      CtrusLogo(size: expandedLogoSize)
  ...
  private func breakStatusSymbol(size: CGFloat) -> some View {
  ...
  private func compactElapsedTimerText(for state: CtrusWidgetAttributes.ContentState) -> some View {
    Text(
      Date(timeIntervalSinceNow: state.getTimeIntervalSinceNow()),
      style: .timer
    )
...
extension CtrusWidgetAttributes {
  fileprivate static var preview: CtrusWidgetAttributes {
    CtrusWidgetAttributes(
      name: "Focus Session",
      message: "Stay focused and avoid distractions")
  }
}

extension CtrusWidgetAttributes.ContentState {
  fileprivate static var shortTime: CtrusWidgetAttributes.ContentState {
    CtrusWidgetAttributes
      .ContentState(
  ...
  fileprivate static var longTime: CtrusWidgetAttributes.ContentState {
    CtrusWidgetAttributes.ContentState(
      startTime: Date(timeInterval: 60, since: Date.now),
  ...
  fileprivate static var breakActive: CtrusWidgetAttributes.ContentState {
    CtrusWidgetAttributes.ContentState(
      startTime: Date(timeInterval: 60, since: Date.now),
  ...
  fileprivate static var pauseActive: CtrusWidgetAttributes.ContentState {
    CtrusWidgetAttributes.ContentState(
      startTime: Date(timeInterval: 60, since: Date.now),
...
#Preview("Notification", as: .content, using: CtrusWidgetAttributes.preview) {
  CtrusWidgetLiveActivity()
} contentStates: {
  CtrusWidgetAttributes.ContentState.shortTime
  CtrusWidgetAttributes.ContentState.longTime
  CtrusWidgetAttributes.ContentState.breakActive
  CtrusWidgetAttributes.ContentState.pauseActive
}
